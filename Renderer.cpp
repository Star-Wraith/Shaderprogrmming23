...
void Renderer::DrawParticleWithBloom()
{
	...
	DrawTexture(-0.25, -0.75, 128, 128, m_HDRHighTexture);*/
	...
	DrawTexture(-0.75, -0.75, 128, 128, m_PingpongTexture[1]);
	DrawTexture(-0.25, -0.75, 128, 128, m_HDRLowTexture);
...
void Renderer::PrepareBloom()
{
	...
	for (int i = 0; i < 2; ++i) {
		...
		status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
		if (status != GL_FRAMEBUFFER_COMPLETE) {
			std::cout << "fbo creation failde" << std::endl;
		}
	}
...
void Renderer::DrawTexture(float x, float y, float scaleX, float scaleY, GLuint texID)
{
	...
	GLuint posSclaeULoc = glGetUniformLocation(shader, "u_PosScale");
	glUniform4f(posSclaeULoc, x, y, scaleX, scaleY);
...
void Renderer::CreateParticles(int numParticles)
{
	...
	for (int i = 0; i < particleCount; i++)
	{
		...
		float r = 5.f *((float)rand() / (float)RAND_MAX);
		float g = 5.f *((float)rand() / (float)RAND_MAX);
		float b = 5.f *((float)rand() / (float)RAND_MAX);
		float a = ((float)rand() / (float)RAND_MAX);
